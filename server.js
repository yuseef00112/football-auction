...
function publicState(r){return {phase:r.phase,round:r.round,bid:r.bid,highest:r.highest,turn:r.turn||null,endsAt:r.endsAt,current:r.current,bids:r.bids,roundPosition:roundPositions[r.round-1],roundLabel:category[roundPositions[r.round-1]],players:Object.fromEntries([...r.players].map(([id,p])=>[id,{name:p.name,budget:p.budget,team:p.team}]))}}
...
function startRound(r){const pos=roundPositions[r.round-1],p=pickForPosition(r,pos);if(!p){finishGame(r);return}r.used.add(p.id);r.current=p;r.bid=1;r.highest=null;r.bids=[];r.endsAt=Date.now()+20000;r.skipUsed=new Set();const ids=[...r.players.keys()];r.turn=ids[Math.floor(Math.random()*ids.length)];broadcast(r,{type:"state",state:publicState(r)});clearTimeout(r.timer);r.timer=setTimeout(()=>finishRound(r),20100)}
...
wss.on("connection",ws=>{
  ...
  ws.on("message",raw=>{
    ...
    if(m.type==="create"){
      room=roomCode();
      const r={phase:"lobby",round:0,players:new Map(),clients:new Set(),used:new Set(),current:null,bid:0,highest:null,bids:[],endsAt:0,timer:null,skipUsed:new Set(),turn:null,replayRequester:null};
      rooms.set(room,r);
      r.players.set(id,{name:String(m.name).slice(0,20),budget:100,team:[]});
      r.clients.add(ws);
      ...
      r.players.set(id,{name:String(m.name||"لاعب").slice(0,20),budget:100,team:[]});
      r.clients.add(ws);
      ...
      if(m.type==="bid"&&r.phase==="auction"){
        ...
        const amt=r.highest?r.bid+amount:1;
        if(p&&p.budget>=amt&&amt>0){
          r.bid=amt;r.highest=id;r.bids.push({name:p.name,amount:amt});
          const ids=[...r.players.keys()];r.turn=ids.find(x=>x!==id)||id;
      ...
      if(m.type==="replayResponse"&&r.phase==="done"&&r.players.size===2){
        ...
        if(m.accept){
          ...
          for(const p of r.players.values()){p.budget=100;p.team=[]}
          startRound(r);
        }else{
