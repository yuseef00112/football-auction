...
class SimpleWebSocket extends EventEmitter{
  constructor(socket){super();this.socket=socket;this.readyState=1;socket.on("data",d=>this._parse(d));socket.on("close",()=>this._close());socket.on("error",()=>this._close());}
  _close(){if(this.readyState===3)return;this.readyState=3;this.emit("close");}
  _parse(buf){let offset=0;while(offset+2<=buf.length){const b1=buf[offset++],b2=buf[offset++];const opcode=b1&15,masked=!!(b2&128);let len=b2&127;if(len===126){if(offset+2>buf.length)return;len=buf.readUInt16BE(offset);offset+=2;}else if(len===127){if(offset+8>buf.length)return;const n=buf.readBigUInt64BE(offset);offset+=8;if(n>BigInt(Number.MAX_SAFE_INTEGER)){this.close();return;}len=Number(n);}let mask;if(masked){if(offset+4>buf.length)return;mask=buf.subarray(offset,offset+4);offset+=4;}if(offset+len>buf.length)return;let payload=Buffer.from(buf.subarray(offset,offset+len));offset+=len;if(masked)for(let i=0;i<payload.length;i++)payload[i]^=mask[i%4];if(opcode===8){this.close();return;}if(opcode===9){this._frame(payload,10);continue;}if(opcode===1)this.emit("message",payload.toString("utf8"));}}
  _frame(payload,opcode=1){if(this.readyState!==1)return;const p=Buffer.isBuffer(payload)?payload:Buffer.from(String(payload));let head;if(p.length<126){head=Buffer.alloc(2);head[0]=128|opcode;head[1]=p.length;}else if(p.length<=65535){head=Buffer.alloc(4);head[0]=128|opcode;head[1]=126;head.writeUInt16BE(p.length,2);}else{head=Buffer.alloc(10);head[0]=128|opcode;head[1]=127;head.writeBigUInt64BE(BigInt(p.length),2);}try{this.socket.write(Buffer.concat([head,p]));}catch(e){this._close();}}
  send(data){this._frame(data,1);}
  close(){if(this.readyState===3)return;try{this._frame(Buffer.alloc(0),8);this.socket.end();}catch(e){}this.readyState=3;this.emit("close");}
}
class SimpleWebSocketServer extends EventEmitter{
  constructor({server}){super();this.clients=new Set();server.on("upgrade",(req,socket)=>{const key=req.headers["sec-websocket-key"];if(!key){socket.destroy();return;}const accept=crypto.createHash("sha1").update(key+"258EAFA5-E914-47DA-95CA-C5AB0DC85B11").digest("base64");socket.write("HTTP/1.1 101 Switching Protocols\\r\\nUpgrade: websocket\\r\\nConnection: Upgrade\\r\\nSec-WebSocket-Accept: "+accept+"\\r\\n\\r\\n");const ws=new SimpleWebSocket(socket);this.clients.add(ws);ws.on("close",()=>this.clients.delete(ws));this.emit("connection",ws,req);});}
}
